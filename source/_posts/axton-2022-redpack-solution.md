---
title: 无垠 2022 跨年红包题解
date: 2023-01-22 14:27:37
tags:
---

## 0x00 题面

题见[无垠 2021 年度总结](https://flyhigher.top/diary/2507.html)  
题面为一张图片,链接为  
[https://acdn.flyhigher.top/wp-content/uploads/2021/12/b.jpg](https://acdn.flyhigher.top/wp-content/uploads/2021/12/b.jpg)  
{% asset_img b.jpg  %}

## 0x01 Step 1

观察得知图片本身只是一张风景照，也并没有 Axton 惯用的隐藏压缩文件之类的。
查看图片属性，发现备注中写着`Original Resolution: 1510x1500`,而这与图片尺寸不符，故需要修改图片高度。
编辑图片十六进制数据，可以用`010 editor`,这里使用 Visual Studio Code 的`HEX Editor`插件
参见[JPEG 图片存储格式及原理\_huangxy10 的博客](https://blog.csdn.net/huangxy10/article/details/8117469)
{% asset_img QQ截图20230122145224.png  %}
直接搜索`FFC0`，各字节信息如下。
{% asset_img QQ截图20230122144712.png  %}
| FF C0        | 00 11          | 08  | 04 E2    | 05 E6    |
| ------------ | -------------- | --- | -------- | -------- |
| SOF 开始标记 | SOF 标记码长度 |     | 图像高度 | 图像宽度 |
我们只需要把 04 E2 修改为 05 DC （也就是1500)即可看到隐藏信息。
{% asset_img b_en.jpg  %}
内容为
`01101000 01110100 01110100 01110000 01110011 00111010 00101111 00101111 01100111 01101111 00110010 00110000 00110010 00110010 01110010 01110000 00101110 01100001 01111000 01110100 01101111 01101110 00101110 01101001 01101101`
解码得
`https://go2022rp.axton.im`

## 0x02 Step 2
{% asset_img QQ截图20230122150931.png  %}
说明
```javascript
// 15 Puzzle Solver
// 使用 Javascript 编写代码，自动求解 15 Puzzle，解出即可获得红包
// 15 Puzzle 的获胜规则应该是不言自明的

// 快速指北：
// - 棋局一定有解，因此你无需额外检查
// - 调用 fps.start() 开始或重启游戏
// - 调用 fps.move(x, y) 移动指定方格（自动匹配可以移动的方向）
// - 或调用 fps.up() fps.down() fps.left() fps.right() 移动方格（自动匹配可以移动的方格）
// - 所有方法的返回值均为 Promise<{ status: boolean | string, data: number[][] }>，其中 data 表示当前棋盘，我们使用 15 标识空格
// - 方法的返回值 status 为 false 时方格移动失败，其他任何值都表示方格移动成功
// - 每次只有当上一个操作完成后才能执行下一个操作，强行移动将会被服务器封禁
// - 解出后页面将自动指导你获取红包口令码

// 代码编写完成后，点按代码编辑器右上角的运行按钮执行代码
// 注：如果使用 const 和 let，你可能无法重复运行代码（因为对应变量已被定义）。你可以通过重置按钮来重置运行环境，代码不会丢失
...
```
题目为一个15 Puzzle，一道经典的OI题，不过语言是JavaScript,且没有直接给出棋盘信息。
一种简单的解决办法是~~Google一下你就知道~~
可以搜到一个轮子，[GitHub:technogeek00/NPuzzleSolver](https://github.com/technogeek00/NPuzzleSolver),自己做一下数据转换就可以解决
```javascript
(async () => {
  ...
  NPuzzleSolver.prototype.solveColumn = function (size) {
    ...
    var secondToLast =
      (this.grid.length - 2) * this.grid.length + 1 + colNumber;
  ...
  NPuzzleSolver.prototype.proper3By2RotationHorizontal = function (
    pos,
    leftDirection
  ) {
    var side = leftDirection ? "l" : "r";
  ...
  NPuzzleSolver.prototype.proper2By3RotationVertical = function (
    pos,
    upDirection
  ) {
    var toward = upDirection ? "u" : "d";
  ...
  NPuzzleSolver.prototype.makeEmptyNeighborTo = function (num, boundry) {
    ...
    var counter = 1;
    while (
      (this.numbers[""].x != gotoPos.x || this.numbers[""].y != gotoPos.y) &&
      !this.areNeighbors("", num)
    ) {
  ...
  const puzzle_start = await fps.start();
  const puzzle_data = puzzle_start.data;
  const new_puzzle = puzzle_data.map((x) =>
    x.map((y) => (y === 15 ? "" : y + 1))
  );
//   console.log(puzzle_data);
//   console.log(new_puzzle);
  const solver = new NPuzzleSolver(new_puzzle);
  const solution = solver.solve();
  console.log(solution);
  for (each of solution) {
      await fps.move(each.piece.x, each.piece.y)
    // console.log(each.piece);
  }
})();
```
{% asset_img QQ截图20230122154854.png  %}
```plain
Well done! Go to 🎉.yrc.me to get your reward!
```
done.